...
describe('AE Tests', () => {
  ...
  it('AE_TestCase15', () => {
    ...
    cy.get('.navbar-nav li:last-child a')
      .should('be.visible')
      .should('contain', `Logged in as ${username}`); //Verify ' Logged in as username' at top
    cy.get('a[data-product-id="1"]').first().click(); //Add products to cart
    cy.get('a[href="/view_cart"]').last().click(); //Click 'View Cart' button
    cy.get('li.active')
      .should('be.visible')
      .should('have.text', 'Shopping Cart'); //Verify that cart page is displayed
    cy.get('a.btn.btn-default.check_out').click(); //Click Proceed To Checkout
    cy.get('h2.heading')
      .first()
      .should('be.visible')
    ...
    cy.get('.form-control').type('Comments'); //Enter description in comment text area
    cy.get('a.btn.btn-default.check_out').click(); //click 'Place Order'
    cy.get('#payment-form > div:nth-child(2) > div > input').type(username); //Enter payment details: Name on Card
    cy.get('#payment-form > div:nth-child(2) > div > input').type('123456789'); //Enter payment details: Card Number
    cy.get('input.form-control.card-number').type('123'); //Enter payment details: CVC
    cy.get('input.form-control.card-expiry-month').type(10); //Enter payment details: Expiration date month
    cy.get('input.form-control.card-expiry-year').type('2030'); //Enter payment details: Expiration date year
    cy.get('#submit').click();
    cy.get('#success_message > div').then((el) => {
      const text = el.show().text().trim();
      expect(text).to.eq('Your order has been placed successfully!');
      cy.get('a[href="/delete_account"]').click(); //Click 'Delete Account' button
      cy.get('.title.text-center')
        .should('be.visible')
        .should('have.text', 'Account Deleted!'); //Verify 'ACCOUNT DELETED!'
      cy.get('.btn.btn-primary').click(); //click 'Continue' button
    });
  });
